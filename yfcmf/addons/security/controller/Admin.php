...
class Admin extends Base
{
	...
	public function security_list()
	{
		...
		foreach ($finger_files as &$finger_file){
		    $finger_file['check_href']=addon_url('security://Admin/security_check',array('file'=>md5($finger_file['filename'])));
            $finger_file['del_href']=addon_url('security://Admin/security_delete',array('file'=>md5($finger_file['filename'])));
        }
        //表格字段
        $fields=[
        ...
        $widget=new Widget();
        return $widget
            ->addToparea([],[['title'=>'重生成安全文件','id'=>'security_generate','type'=>'a','attr'=>['class'=>'btn btn-xs btn-danger','icon_l'=>'ace-icon fa fa-bolt bigger-110','href'=>addon_url('security://Admin/security_generate')]]])
	...
	public function security_check()
	{
		...
		if (null != $md5_file) {
			...
			if (count($lines) < 3) {
				$this->error('安全文件错误1',addon_url('security://Admin/security_list'));
			}
			if (!preg_match('/^GENE: RCF V.*?$/', $lines [0]) || !preg_match('/^TIME: \\d+\\-\\d+\\-\\d+ \\d+:\\d+:\\d+$/', $lines [1]) || !preg_match('/^ROOT: ([\\/\\.]*)/', $lines [2])) {
				$this->error('安全文件错误2',addon_url('security://Admin/security_list'));
			}
			...
			if(!$error_msgs) $error_msgs []['error']='当前系统没有发生文件被修改问题。';
            //表格字段
            $fields=[
            ...
            $widget=new Widget();
            return $widget
                ->addToparea([],[['title'=>'返回安全文件列表','type'=>'a','attr'=>['class'=>'btn btn-xs btn-primary','icon_l'=>'ace-icon fa fa-rotate-left bigger-110','href'=>addon_url('security://Admin/security_list')]],['title'=>'重生成安全文件','id'=>'security_generate','type'=>'a','attr'=>['class'=>'btn btn-xs btn-danger','icon_l'=>'ace-icon fa fa-bolt bigger-110','href'=>addon_url('security://Admin/security_generate')]]])
