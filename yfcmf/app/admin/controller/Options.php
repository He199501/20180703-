...
class Options extends Base
{
    ...
    {
        $data=input('');
        OptionsModel::setOptions($data,$this->lang);
        OptionsModel::delCache('','base',$this->lang);
    ...
    public function langIndex()
    {
        $widget=new Widget();
        return $widget
            ->addSwitch('lang_switch_on','是否开启多语言',config('lang_switch_on'))
            ->addText('default_lang','默认多语言',config('default_lang'))
    ...
    {
        $sys=OptionsModel::getOptions('email_options',$this->lang);
        $options=[
            ''=>'普通连接方式',
            'ssl'=>'SSL连接方式',
            'tls'=>'TLS连接方式'
    ...
    public function emailUpdate()
    {
        if (!request()->isAjax()){
            ...
            $data=input('');
            OptionsModel::setOptions($data,$this->lang);
            OptionsModel::delCache('','email_options',$this->lang);
    ...
    {
        $sys=OptionsModel::getOptions('active_options',$this->lang);
        $widget=new Widget();
        return $widget
    ...
    public function activeUpdate()
    {
        if (!request()->isAjax()){
            ...
            OptionsModel::setOptions($data,$this->lang);
            OptionsModel::delCache('','email_active',$this->lang);
    ...
    public function cacheUpdate()
    {
        ...
        //保存
        $rst=sys_config_setbyarr(array('cache'=>$cache));
        if($rst){
            Cache::clear();
