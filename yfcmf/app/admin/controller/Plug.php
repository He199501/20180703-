...
class Plug extends Base
{
	...
	public function linkIndex()
	{
        ...
        $lang && $map[]=['lang','=',$lang];
        if(!config('lang_switch_on')) $map['lang']=['lang','=',$this->lang];
	...
	public function linkAdd()
	{
        ...
        return $widget
            ...
            ->addSelect('lang','语言',['zh-cn'=>'中文','en-us'=>'英语'],'zh-cn','','required')
    ...
    public function linkEdit()
    {
        ...
        return $widget
            ...
            ->addSelect('lang','语言',['zh-cn'=>'中文','en-us'=>'英语'],$link['lang'],'','required')
    ...
    public function linkUpdate()
    {
        ...
        $rst=$this->link_model->update($sl_data);
        if($rst!==false){
	...
	public function linkState()
	{
        ...
        if(!$id) $this->error('友情链接不存在','linkIndex');
...
*/
    public function linktypeUpdate()
    {
        ...
        $rst=$this->linktype_model->update($sl_data);
        if($rst!==false){
	...
	public function linktypeDel()
	{
	    ...
	    if($rst) $this->error('友链栏位存在友链,请先删除友链','linkIndex');
    ...
    public function adEdit()
    {
        ...
        return $widget
            ->addText('id','',$id,'','','hidden')
            ->addSelect('adtypeid','广告位',$adtype_list)
            ->addSelect('lang','语言',[''=>'选择语言','zh-cn'=>'中文','en-us'=>'英语'],$ad['lang'])
            ->addText('name','广告名称',$ad['name'],'','required','text',['placeholder'=>'输入广告名称'])
    ...
    */
    public function adUpdate()
    {
        ...
        $rst=$this->ad_model->update($sl_data);
        if($rst!==false){
	...
	{
        $id=input('id',0,'intval');
        if(!$id) $this->error('广告不存在','adIndex');
    ...
    {
        $rst=$this->adtype_model->update(input('post.'));
        if($rst!==false){
	...
	public function adtypeDel()
	{
	    ...
	    if($rst) $this->error('请先删除该广告位的广告','adIndex');
		if($rst!==false){
 ...
 */
    public function sourceUpdate()
    {
        ...
        $rst=$this->source_model->update($sl_data);
        if($rst!==false){
