...
class TimeForm
{
    ...
    public function fetch($name,$title,$default='',$help_text='',$attr=[],$extra_class='col-xs-10 col-sm-5',$extra_attr='')
    {
        ...
        $html .= '<input type="text" name="' . $data['name'] . '" value="' . $data['value'] . '" id="time-'.$data['name'].'" class="time-picker ' . $data['extra_class'] . '" style=" ' . $data['extra_attr'] . '" placeholder="'.$data['attr']['placeholder'].'" '.($data['attr']['disabled']?'disabled="disabled" ':''). ' '.($data['attr']['tips']?'data-rel="tooltip" data-placement="bottom" data-original-title="'.$data['attr']['tips'].'" ':'').' />';
        if($data['help_text']){
