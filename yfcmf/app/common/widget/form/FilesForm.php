...
class FilesForm
{
    ...
    public function fetch($name='',$title='',$default='',$help_text='',$attr=[],$extra_class='col-xs-10 col-sm-5')
    {
        ...
        $html .= '<div class="upload-files no-padding '.$data['extra_class'].'">';
        ...
        $html .='</ul>';
        $html .='<input type="hidden" name="'.$data['name'].'" data-multiple="true" data-url="'.$data['attr']['url'].'" data-size="'.$data['attr']['size'].'" data-ext="'.$data['attr']['ext'].'" id="'.$data['name'].'" value="'.$data['value'].'">';
        $html .='<div id="picker_'.$data['name'].'">上传多个文件</div></div>';
        if($data['help_text']){
